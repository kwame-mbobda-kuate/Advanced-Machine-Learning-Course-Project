...
# our answer set
answer_set = set()
path = r"src\solver\train-00000-of-00001.parquet"

df = pd.read_parquet(path)
for line in df["answer"]:
    # Convertir en majuscules et ne garder que A-Z
    w = "".join(
            [
                c.upper()
...
class BPSolver(Solver):
    def __init__(self, crossword, max_candidates=500000, process_id=0, **kwargs):#constante
        super().__init__(
    ...
    def get_candidate_replacements(self, uncertain_answers, grid):
            ...
            for flip in clue_flips:
                ...
                for i in range(len(flip)):
                    if flip[i] != initial_word[i]:
                        candidate_replacements.append([(cells[i], flip[i])]) #c'est une liste de liste d'un tuple (bpcellule et une lettre )
                        break
        ...
        for cell_id, cell in enumerate(self.bp_cells):
            ...
            above_threshold = list(probs > 0.01) # constante
            new_characters = [
        ...
        for cr in candidate_replacements:
            ...
            variables = set(sum([cell.crossing_vars for cell, _ in cr], [])) #  variables toucher par les modifs
    ...
    def iterative_improvement(self, grid):
        ...
        for replacements in self.candidate_replacements:
            ...
            if modified_grid_score - original_grid_score > 0.5:# constante
